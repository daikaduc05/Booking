...
public interface IFormBookingRepository extends JpaRepository<FormBooking, Integer> {
    @Query(value = "SELECT new org.example.bookingphoto.dto.FormBookingShowDTO(fb.id, fb.location, fb.email, fb.name, fb.message, fb.createAt, fb.bookTime, fb.status, p.id, p.name, p.price) from FormBooking as fb left join Package as p on fb.packageField.id = p.id")
